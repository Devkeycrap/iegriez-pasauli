...
export class Words extends Component<WordProps> {
  ...
  compare(pos) {                           
    ...
    _.map(obj => {
      if (floor(obj.pos.x) == floor(pos.x) || 
      floor(obj.pos.y) == floor(pos.y)) return;
      

      if (obj.pos.x - (obj.age/2) > pos.x && obj.pos.x + (obj.age/2) < pos.x) return
      return valid = true;
    })
    return valid;
  }
        ...
        <motion.div variants={stagger} className={styles.container}>
          ...
          <div className={styles.canvas} onClick={(e) => this.clickEvent(e)}>
            ...
            {this.state.currActors.map((obj, i) => {
              return (
                <label id={obj.date} style={{left: obj.pos.x, top: obj.pos.y, fontSize: obj.age+"px"}}>{obj.content}</label>
              )
            })}
          </div>
