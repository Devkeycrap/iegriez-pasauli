...
interface MapProps {
    setStage: (stage: number) => void;                              // Import props from parent code
  }
  
export default class Words extends Component<MapProps> {
    
    state = {                                                       // Declare class variables
        inBrowser: false,                                           // variable - this.state.inBrowser
        points: 0,                                                  // variable - this.state.points
        speed: 0,                                                   // variable - this.state.speed
        wordList: {                                                 // Word dictionary, with predefined correct awnsers

            correct:["remontē", "salabo", "sašuj", "salāpi", 
        "šķiro", "atdod", "aizņemies", "iestādi", "audzē", "pārstrādā",
        "ēd vietējo", "samal"], 

            dictionary:["remontē", "salabo", "sašuj", 
        "nešķiro atkritumus", "salāpi", "šķiro", "atdod", "aizņemies", 
        "tērē ūdeni", "iestādi", "audzē", "pērc jaunu", "izmanto ķīmiju",
        "pārstrādā", "ēd vietējo", "samal"]

        }
    }

    componentDidMount() {                                           // Function *only* runs on first render
        this.setState({ inBrowser: true })                          // Set inBrowser: true, for later validation NoSSR
    }
                                                                
    newObj(parent) {                                                // Function for creating new componenet(word)
        if (true) {                                                 // Redundant if statment, seems to speed up the code, for no appearent reson, but if it works it works.
            const obj = document.createElement("label");            // Create blank html <label/> element

            obj.innerHTML = this.state.wordList.dictionary[Math.floor(Math.random()*this.state.wordList.dictionary.length)];    // Style <label/> element
    ...
    newTick(elements) {                                                                                 // Function for updating existing component(word) age/size.
        for (let i = 0; i < elements.length; i++) {
            let element = elements[i];                                                                  // loop trough each existing obj in the current tick
            let wordSize = parseInt(window.getComputedStyle(element).getPropertyValue("font-size"));    // Get fontsize and parse to Int
    ...
    clickEvent(e) {                                                                         // Function for handling click events inside of game canvas
        let correct = false;                                                                // Variable for 

        this.state.wordList.correct.map((word, i) => {                                      // Compare if clicked on target has 
        ...
        if (correct) {                                                                      // If compare loop returns, true, 
    ...
    render() {
        ...
        const mainThread = setInterval(() => {                                                     // Main interval for 
